...
기준일: 2026-07-29<br>
개편 작업 브랜치: `document_ai_lecture_2026`<br>
배포 브랜치: `master`

이 문서는 1~8교시 교재, Colab 실습, 이미지 자산의 범위와 완료 증거를 추적한다.

> 2026-07-29 기술 정정 완료: 활성 과정은 PaddleOCR 3.7.0,
...
이 과정은 Python 기초 경험만 있는 Document AI 입문자를 위한 8교시
과정이다. 교시당 60분을 모두 운영하면 접촉시간은 480분이며, 점심과
휴식은 별도다. 전문 지식을 많이 전달하는 것보다 학습자가 작은 문서
자동화 흐름을 직접 완성하는 것을 우선한다.
...
| 3교시 | `clean_receipt.json` | 현재 사진에 실제 PP-OCRv5를 실행하고 좌표로 행과 품목을 정리 |
...
| 5교시 | `app_05.py` | 이미지 업로드·원본·예제 JSON이 있는 최소 웹앱 열기 |
| 6교시 | `receipt.json`, `app_06.py` | 현재 사진 → 실제 PP-OCRv5 → JSON → 앱 버튼 연결 |
| 7교시 | `receipt_result.xlsx` | 오류 차단·사람 확인 후 3시트 Excel 저장 |
| 8교시 | VLM Markdown·PoC 카드·Office ZIP | 업무 문서 사진에 실제 PaddleOCR-VL 실행 후 PoC 조건 설계 |
...
| 개발 브랜치 게시 | 완료 | 개편 이력과 실제 Colab 8/8 기준선 보존 |
| 5회 최종 품질 사이클 | 완료 | 메시지·실습·문체·운영·배포 회귀검사 |
| `master` 배포 | 완료 | 병합 `23468cf`, OCR 좌표 수정 `5d22c39`, 전체 회귀검사·원격 자산 확인 |
| 현장 실행 사전점검 | 강의 전 수행 | Windows·비로그인 UI·현장 20명 네트워크 확인 |
