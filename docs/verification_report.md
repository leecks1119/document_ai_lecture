...
최신 저장소 코드는 자동검증과 로그인된 실제 Colab 8개 전체 실행 기준
**GO**다. Windows 녹화 PC와 비로그인 접근은 외부 사전 점검 항목으로 남긴다.
...
| 실제 로그인 Colab 전체 실행 | 커밋 `4313c29`, 8/8 통과 |
| 실제 PP-OCRv5 LIVE | 46영역, OCR JSON·박스 이미지 생성, 약 3분 15초 |
...
각각 `모두 실행`했다. 2교시는 PaddleOCR 3.7.0과 PP-OCRv5 Korean LIVE로
46영역을 판독했다. 이 결과와 별도로 6교시의 44토큰 자료는 이전 LIVE
실행을 보존한 회귀 fixture이며, 공간 순서 복원·총액 76,000원·품목 5개가
...
- 검증 기능·교재 커밋 `4313c29` 원격 push·SHA 일치 확인 완료
- 비로그인 Colab 8개 접근
