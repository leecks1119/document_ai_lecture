...
실행은 커밋 `4313c29`에서 통과했고, 이후 변경은 교재 문장·배포 링크와
7교시 앱 소스 포장 방식을 읽기 쉽게 줄인 부분이다. 최종 배포본은 아래
자동 회귀검사를 다시 통과해야 한다. Windows 녹화 PC와 비로그인 접근은
외부 사전 점검 항목으로 남긴다.
...
| 실제 PP-OCRv5 직접 실행 | 46영역, OCR JSON·박스 이미지 생성, 약 3분 15초 |
...
| OCR 박스 좌표 회귀 | 실제 기록 좌표계 900×1003과 원본 2558×2850의 종횡비·해시·좌표 범위를 함께 검사 |
...
실행해 46영역을 판독했다. 이 결과와 별도로 6교시의 44토큰 자료는 이전
