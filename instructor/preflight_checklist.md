...
- [x] 2교시 `RUN_OCR_NOW=True` 경로를 공개 한국 영수증으로 실행했다.
- [x] 직접 실행 결과의 `source_mode`, 인식 영역 수, 총 실행 시간을 기록했다.
- [ ] 실패했다면 오류 종류와 `COURSE_EXAMPLE` 전환 화면을 캡처했다.
...
| 결과 | `직접 실행`, 인식 영역 46개, 최신 8교시 전체 체크포인트 통과 |
