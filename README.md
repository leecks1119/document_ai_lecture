...
문서 이미지 한 장으로 같은 실험을 반복할 수 있습니다. 2교시에서
`USE_MY_FILE`만 `True`로 바꾸어 파일을 선택하면 OCR 상자·인식 글자·신뢰도
표가 바로 표시됩니다. 내려받은 `ocr_result.json`은 3~7교시에 그대로 이어
쓰며, 자료 선택과 개인정보 확인 방법은
