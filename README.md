...
각 코드 셀을 실행하면 출력의 맨 위에 `현재 실습 단계`, `지금 할 일`,
`확인할 결과`가 표시됩니다. 셀이 정상적으로 끝나면 `단계 실행 완료`와
다음 행동이 이어집니다. 따라서 코드 전체를 먼저 이해하려 하지 말고,
현재 단계의 안내를 읽고 출력 결과 한 가지를 확인한 뒤 다음 셀로 이동합니다.
