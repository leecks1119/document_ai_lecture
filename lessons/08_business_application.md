...
- `poc_candidate_card.md`와 `office_format_samples.zip`을 만듭니다.
...
Colab은 실제 Excel·Word·PDF·PPT 파일을 `office_format_samples.zip` 하나로 제공합니다. 각 파일을 열기 전에 내부 구조 검사 결과를 보고, “원본 구조 직접 읽기”와 “이미지 판독” 중 무엇을 먼저 쓸지 판단합니다.

![추출과 검증 뒤 사람 확인을 거쳐 Excel이 확정되는 흐름](assets/08/01_human_review.svg)

### 3. 내 PoC 카드를 완성합니다

`MY_CANDIDATE`, 점수, 검토자, 중단 조건은 빈칸입니다. 견적서·신청서·거래명세서 중 실제 업무에 가까운 하나를 고르고 자기 답을 먼저 실행합니다. 막히면 바로 아래 힌트와 전체 정답으로 복구합니다.
...
- 자신의 선택과 점수가 반영된 PoC 카드가 있습니다.
- 실제 Office 4종 파일의 내부 구조 차이를 확인했습니다.
