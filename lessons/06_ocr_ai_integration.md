...
> **이번 시간의 도착점:** 업로드한 문서 한 장을 OCR·읽기 순서 복원·정보 추출에 연결하고 직접 읽기·실패·수업용 예제 상태를 구분합니다.
...
앱의 `read_receipt_now` 함수는 내부에서 사용하는 함수 이름입니다. 이 함수는
업로드된 파일을 임시로 저장하고 PaddleOCR에 전달합니다. 수강생 화면에는
함수 이름 대신 `내 영수증 직접 읽기` 버튼이 보입니다.
...
사용자가 올린 파일에 OCR과 정보 추출을 실행한 상태입니다. 화면에는 다음과
같이 표시됩니다.

> 지금 이 사진을 OCR로 직접 읽었습니다.

### 사진을 직접 읽지 못한 상태

파일이 없거나, 모델을 불러오지 못했거나, 결과를 JSON으로 바꾸지 못한
상태입니다. 화면에는 `사진을 직접 읽지 못했습니다`와 실패 이유가 표시됩니다.
오류가 발생했는데 이전 예제 결과를 성공한 것처럼 보여 주면 안 됩니다.

### 수업용 예제 결과

네트워크나 모델 다운로드 문제로 수업 진행이 어려울 때 사용자가 직접 선택하는
검수 예제입니다. 화면에는 다음과 같이 표시됩니다.

> 수업용 예제 결과를 불러왔습니다. 현재 업로드한 파일을 분석한 결과가 아닙니다.

![현재 파일 처리 오류를 표시한 뒤 사용자가 수업 예제를 선택하는 흐름](assets/06/01_processing_paths.svg)
...
먼저 `수업용 예제로 계속하기` 버튼을 눌러 다음 영역이 모두 나타나는지 확인합니다.
...
노트북의 `웹앱·OCR 실행환경 준비` 셀이 끝나면 공개 영수증을 업로드하고
`내 영수증 직접 읽기` 버튼을 누릅니다. 이 셀은 Streamlit과 PaddleOCR을
확인하고, 필요한 경우 설치합니다. 설치가 실패하면 오류 이유를 남기고
`수업용 예제로 계속하기`로 실습을 계속합니다.
