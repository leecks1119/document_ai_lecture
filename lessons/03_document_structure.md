...
OCR이 반환한 토큰 배열 순서는 사람이 읽는 순서와 다를 수 있습니다. 먼저 바운딩 박스의 세로 위치로 같은 줄을 묶고, 그 안에서 가로 위치로 정렬해야 `품목명·단가·수량·금액` 관계를 보존할 수 있습니다. 그다음 공백과 표기를 정리하며, 원문에 없는 상호명이나 금액을 새로 만들지 않습니다.
...
품목 줄을 찾는 정규식은 빈칸으로 제공됩니다. 먼저 “줄 끝에 숫자·숫자·숫자가 연속되는가?”를 자기 코드로 표현하고, 힌트와 전체 정답으로 복구합니다.
