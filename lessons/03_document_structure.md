...
2교시 결과를 이어서 처리하려면 `2교시 결과 파일 업로드`를 고르고,
내려받은 ZIP을 푼 뒤 `ocr_result.json`을 선택합니다. 공개 JSON의 주소가
있다면 `인터넷 JSON URL`을 고를 수도 있습니다. 같은 런타임의
`course_outputs`에 파일이 남아 있으면 그 파일을 먼저 사용합니다.
