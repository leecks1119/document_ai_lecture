...
- AppTest로 자동 검사하고 Colab 안의 비공개 미리보기로 직접 눌러 봅니다.
...
`APP_TITLE`과 `ACTION_LABEL` 빈칸에 자신의 업무를 떠올릴 수 있는 문구를 적습니다. 전체 정답은 바로 아래에 공개되어 있으며, 틀려도 완성 앱으로 복구할 수 있습니다.

검사가 끝나면 Colab 출력 영역의 Streamlit 미리보기를 열어 파일 선택과 버튼을 직접 눌러 봅니다. 이 주소는 Colab 런타임 안의 임시 화면이며 공개 배포 주소가 아닙니다.
...
- Colab 미리보기에서 자신이 바꾼 제목과 버튼을 확인했습니다.
