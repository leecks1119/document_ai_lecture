...
4교시 결과를 이어서 검증하려면 `4교시 결과 파일 업로드`를 고르고
`receipt.json`을 선택합니다. 공개 JSON의 주소가 있다면
...
제공된 한국 영수증을 앞 교시부터 이어 왔다면 실제 OCR가 틀리게 읽은 품목명이
화면에 나타납니다. 노트북은 `OCR 판독 → 원본 대조 후` 수정 내용을 먼저
보여 주고 공개 정답을 적용한 뒤 Excel을 만듭니다. 내 사진이나 다른 공개
자료를 사용했다면 공개 정답을 대신 적용하지 않습니다. 내가 검토 기록을
채우고 최종 앱의 표에서 값을 직접 고쳐야 다운로드가 열립니다.
