...
DIAGRAMS = {
    "01/01_pipeline_map.svg": flow_diagram(
        "문서가 업무 데이터가 되는 5단계",
        "영수증이 OCR 텍스트, JSON, 검증 결과, CSV로 변하는 다섯 단계",
        [
            ("문서", "합성|영수증"),
            ("OCR", "글자|초안"),
            ("JSON", "필드별|정리"),
        ...
        "OCR은 두 번째 단계일 뿐, 전체 자동화는 다섯 단계를 모두 포함합니다.",
    ),
    ...
    "04/01_receipt_to_json.svg": comparison_diagram(
        "영수증에서 JSON으로",
        "영수증의 상호명, 날짜, 품목, 합계가 JSON 필드로 연결되는 그림",
        "문서 영역",
        ["상호명", "거래일자", "품목 두 줄", "합계"],
        "JSON 필드",
        ['"store_name"', '"date"', '"items": [...]', '"total_amount"'],
        "원문에서 찾지 못한 값은 추측하지 않고 null로 둡니다.",
    ),
    "04/02_three_checks.svg": flow_diagram(
        "JSON 결과의 세 가지 확인",
        "구조화 결과가 문법, 자료형, 원문 근거 검사를 차례로 통과하는 그림",
        [
            ("JSON 문법", "괄호·쉼표"),
            ("자료형", "날짜·정수"),
            ("원문 근거", "문서와 비교"),
        ],
        "모양이 맞는 JSON이라도 값이 정확한지는 별도로 확인합니다.",
    ),
    ...
    "05/02_minimal_ui.svg": comparison_diagram(
        "최소 Gradio 화면",
        "파일 입력, 처리 버튼, OCR 텍스트, JSON 결과로 구성된 화면",
        "입력 영역",
        ["파일 선택", "업로드 문서 처리", "샘플로 계속"],
        "결과 영역",
        ["처리 상태", "OCR 텍스트", "JSON·품목 표"],
    ...
    "06/02_status_steps.svg": comparison_diagram(
        "처리 상태를 숨기지 않기",
        "문서 처리 각 단계가 실제 또는 mock 사용으로 표시된 상태 화면",
        "LIVE 선택 경로",
        ["파일 확인 완료", "EasyOCR 실행", "MOCK JSON 추출"],
        "MOCK 기본 경로",
        ["업로드를 읽지 않음", "샘플 OCR 사용", "MOCK JSON 추출"],
        "사용자는 어떤 단계가 실제이고 mock인지 항상 알 수 있어야 합니다.",
    ),
